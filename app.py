...
def index():
    if request.method == "POST":
        try:
            ...
            file_obj = request.files.get("up_file") # 这是一个 filestorage类型
           ...
           '''
            ...
            return render_template("index.html", msg=json.dumps(".mat文件读取失败！\n请检查数据是否正确。"), userid='0')
        try:
        ...
        return render_template("index.html", fc_info = fc_info, file_name = json.dumps(file_name), userid = userid)
    return render_template("index.html", userid='0')


@app.route('/getH2', methods=['GET'])
def getH2():
    if request.method == "GET":
        try:
            ...
            h2_threshold = float(request.args.get('h2_threshold'))
            select_start = float(request.args.get('select_start'))
            select_end = float(request.args.get('select_end'))

            global matDataList
            if userid not in matDataList:
                return jsonify({'result': False, "msg": "会话已过期，请刷新界面或重新上传mat文件！"})
            ...
            return jsonify({'result': False, 'msg': "系统出错！\n" + repr(e)})
        return jsonify({'result': True, 's1_s2': s1_s2})
    return jsonify({'result': False, 'msg': "Not GET Request!"})


@app.route('/fcAnalyse', methods=['POST'])
def fcAnalyse():
    '''
    区域内与区域间的FC分析
    :param request:
    ...
    if request.method == "POST":
        try:
            ez = json.loads(request.form.get('ez'))
            pz = json.loads(request.form.get('pz'))
            niz = json.loads(request.form.get('niz'))
            ...
            h2_threshold = float(request.form.get('h2_threshold'))
            select_start = float(request.form.get('select_start'))
            select_end = float(request.form.get('select_end'))
            select_s = int((select_start - start) / step)  # 筛选的起始时间下标
            select_l = int((select_end - start) / step) - select_s + 1  # 筛选的总长度
        ...
        # return jsonify({'result': True, "ez":ez_in, "pz":pz_in, "niz":niz_in, "ez_pz":ez_pz, "ez_niz":ez_niz, "pz_niz":pz_niz})   # 'zones': [ez,pz,niz]
        return jsonify({'result': True, "data":[ez_in, pz_in, niz_in, ez_pz, ez_niz, pz_niz]})   # 'zones': [ez,pz,niz]
    return jsonify({'result': False, 'msg': "Not POST Request!"})


@app.route('/distanceAnalyse', methods=['POST'])
def distanceAnalyse():
    '''
    ...
    if request.method == "POST":
        try:
            ...
            distance_interval = float(request.form.get('distance_interval'))
            elects_distance = json.loads(request.form.get('elects_distance'))  # [zone,[s1,s2],distance]
            h2_threshold = float(request.form.get('h2_threshold'))
            select_start = float(request.form.get('select_start'))
            select_end = float(request.form.get('select_end'))
            select_s = int((select_start - start) / step)  # 筛选的起始时间下标
        ...
        return jsonify({'result': True, 'distance_group': distance_group})
    return jsonify({'result': False, 'msg': "Not POST Request!"})


@app.route('/outAnalyse', methods=['GET'])
def outAnalyse():
    if request.method == "GET":
        try:
            select_start = float(request.args.get('select_start'))
            ...
            if userid not in matDataList:
                return jsonify({'result': False, "msg": "会话已过期，请刷新界面或重新上传mat文件！"})
            start, step = matDataList[userid]['start'], matDataList[userid]['step']
            select_ei = json.loads(request.args.get('select_ei'))  # 将字符串格式转化为json对象
            select_s = int((select_start - start)/step)  # 筛选的起始时间下标
            select_l = int((select_end - start)/step) - select_s + 1  # 筛选的总长度
            links = out_in(userid, select_s, select_l, h2_threshold, select_ei)  # [out_links, in_links]
        except Exception as e:
            return jsonify({'result': False, 'msg': "系统出错！\n" + repr(e)})
        return jsonify({'result': True, 'out_links': links[0], "in_links": links[1]})
    return jsonify({'result': False, 'msg': "Not GET Request!"})
...
def out_in(userid, select_s, select_l, h2_threshold, select_ei):
    ...
    # jsonify({'out': out_links, 'in': out_links})
...
@app.route('/ViolinBox')
def ViolinBox():
    return render_template("ViolinBoxPlot.html")


@app.route('/exitSystem', methods=['GET'])
def exitSystem():
    if request.method == "GET":
        userid = request.args.get("userid")
        global matDataList
